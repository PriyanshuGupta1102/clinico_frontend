...
const PatientDashboard = () => {
  ...
  const fetchAppointments = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get(`${API_URL}/api/appointments/${user.id}/patient`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.data.success) {
  ...
  const fetchMedicalRecords = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get(`${API_URL}/api/medical-records/${user.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.data.success) {
  ...
  const handleReportUpload = async (e) => {
    ...
    try {
      ...
      reader.onloadend = async () => {
        try {
          const token = localStorage.getItem('token');
          const res = await axios.post('${API_URL}/api/medical-records', {
            patientId: user.id,
            patientName: user?.name || user?.firstName || 'Patient',
